...
import pylot.flags
#import pylot.component_creator_with_fake  # noqa: I100
import pylot.component_creator
import pylot.operator_creator
import pylot.operator_creator_with_fake
import pylot.utils
...
flags.DEFINE_bool('log_trajectories_fake', False,
                  'True to enable trajectory logging')
...
def driver():
    ...
    obstacles_stream_fake = pylot.component_creator.add_obstacle_detection_fake(
        obstacles_stream)
    if FLAGS.log_obstacles_fake:
        pylot.operator_creator.add_bounding_box_logging(
            obstacles_stream_fake, 'bboxes_fake')

    obstacles_tracking_stream_fake = pylot.component_creator.add_obstacle_tracking(
    ...
    prediction_stream_fake, prediction_camera_stream_fake, notify_prediction_stream_fake = \
        pylot.component_creator.add_prediction(
            obstacles_tracking_stream_fake, vehicle_id_stream,
            time_to_decision_loop_stream, transform, release_sensor_stream,
            pose_stream, point_cloud_stream, lidar_setup)
    if prediction_stream is None:
        prediction_stream_fake = obstacles_stream_fake
    if notify_prediction_stream:
