...
import pylot.operator_creator_safebench
import pylot.utils
...
import os
import yaml
from SafeBench.safebench.carla_runner_pylot import CarlaRunner
...
def driver():
    ...
    ) = pylot.operator_creator_safebench.add_simulator_bridge(
        control_loop_stream,
...
def safebench_init():
    safebench_config = {
        'exp_name': 'exp',
        'output_dir': 'log_safebench',
        'ROOT_dir': os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))), 'pylot/SafeBench'),
        'max_episode_step': 300,
        'mode': 'eval',
        'agent_cfg': 'dummy.yaml',
        'scenario_cfg': FLAGS.scenario_cfg,
    ...
    with open(os.path.join(safebench_config['ROOT_dir'], 'safebench/scenario/config', safebench_config['scenario_cfg']), 'r') as f:
        scenario_config = yaml.safe_load(f)
    scenario_config.update(safebench_config)
    return scenario_config


def safebench_running():
    safebench_config = safebench_init()
    safebench_runner = CarlaRunner(safebench_config)
