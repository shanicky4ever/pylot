...
class CarlaCameraDriverOperator(erdos.Operator):
    ...
    def __init__(self, ground_vehicle_id_stream: erdos.ReadStream,
        ...
        self.mutate_methods = {
            'rgb': {
                'none': CameraFrame.from_simulator_frame,
                'gaussian': CameraFrame.from_simulator_frame_with_noise,
                'broke_part': CameraFrame.from_simulator_frame_with_broke,
                'motion_blur': CameraFrame.from_simulator_frame_with_motion_blur,
                'brightness': CameraFrame.from_simulator_frame_with_brightness,
            },
            'depth': {
                'none': DepthFrame.from_simulator_frame,
                'gaussian': DepthFrame.from_simulator_frame_with_noise,
                'broke_part': DepthFrame.from_simulator_frame_with_broke
            },
        }
        ...
        with erdos.profile(self.config.name + '.process_images',
            ...
            with self._lock:
                ...
                if self._camera_setup.camera_type == 'sensor.camera.rgb':
                    framefunc = self.mutate_methods['rgb'][self._flags.mutate_camera_type]
                    msg = FrameMessage(
                        timestamp,
                        framefunc(
                    ...
                    framefunc = self.mutate_methods['depth'][self._flags.mutate_depth_camera_type]
