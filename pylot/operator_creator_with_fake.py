...
def add_simulator_bridge(control_stream, sensor_ready_stream,
                         pipeline_finish_notify_stream, extern_configs=None):
...
def add_simulator_bridge_safebench(control_stream, sensor_ready_stream,
                         pipeline_finish_notify_stream, extern_configs=None):
    from pylot.simulation.carla_operator_safebench import CarlaOperator
    op_config = erdos.OperatorConfig(name='simulator_bridge_operator',
                                     flow_watermarks=False,
                                     log_file_name=FLAGS.log_file_name,
                                     csv_log_file_name=FLAGS.csv_log_file_name,
...
def add_obstacle_detection(camera_stream,
                           time_to_decision_stream,
                           csv_file_name=None):
    from pylot.perception.detection.detection_operator import DetectionFakeOperator as DetectionOperator
    obstacles_streams = []
    if csv_file_name is None:
        csv_file_name = FLAGS.csv_log_file_name
...
def add_camera_driver(camera_setup, vehicle_id_stream, release_sensor_stream):
    from pylot.drivers.my_carla_camera_driver_operator import \
...
def add_mpc_safebench(pose_stream, waypoints_stream):
    from pylot.control.mpc.mpc_operator import MPCOperator
    op_config = erdos.OperatorConfig(name='mpc_operator',
                                     log_file_name=FLAGS.log_file_name,
                                     csv_log_file_name=FLAGS.csv_log_file_name,
...
def add_multiple_object_tracker_logging(
    ...
    from pylot.loggers.multiple_object_tracker_logger_operator import MultipleObjectTrackerFakeLoggerOperator as\
