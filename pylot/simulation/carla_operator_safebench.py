...
class CarlaOperator(erdos.Operator):
    ...
    def __init__(
            ...
            global_trajectory_stream: WriteStream, flags, extern_configs=None):
        ...
        self._flags = flags
        self.extern_configs = extern_configs
        ...
        if self._flags.scenario_runner or self._flags.control == "manual":
            ...
            self._ego_vehicle = pylot.simulation.utils.wait_for_ego_vehicle(
                self._world, self.extern_configs['ego_name'])
            self._logger.info("Found ego vehicle")
        else:
            start_info = self._flags.simulator_spawn_point_index \
                if self._flags.simulator_spawn_point_index >= 0 else self.extern_configs['start_loc']
    ...
    def _initialize_world(self):
        ...
        if self._simulator_version == '0.9.5':
            ...
            if self._flags.simulator_town > 0:
                self._world = self._client.load_world('Town{:02d}'.format(
                    self._flags.simulator_town))
            else:
                self.world = self._client.load_world(self._flags.simulator_town_name)
