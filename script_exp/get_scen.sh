...
echo "open simulator"
sleep 5s |pv -t
cd $PYLOT_HOME/../scenario_runner
echo "run scenario"
sleep 2s | pv -t
nohup python3 scenario_runner.py --scenario ${name} --reloadWorld > /tmp/scenario.log 2>&1 &
cd $PYLOT_HOME


export CUDA_VISIBLE_DEVICES=$pylot_device
timeout 180s python $PYLOT_HOME/pylot_with_fake.py --flagfile $PYLOT_HOME/configs/scen.conf
